...
Models also have a `type:String` field that cannot be changed after instantiation. This is the type of the model. It is also stored as a static `TYPE` field. The TYPE of a model can be manually set by declaring a public static final TYPE field with a different value. In the following example, the TYPE of Person is "Human".
```haxe
class Person extends olib.models.Model
{
